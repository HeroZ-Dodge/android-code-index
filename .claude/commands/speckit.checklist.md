...
5. **生成清单** - 创建"需求的单元测试": 
   ...
   - Generate checklist filename based on domain:
     ...
     - Format: `[domain].md`
   - **文件策略**:
     - 如果 `FEATURE_DIR/checklists/[domain].md` 已存在: 在文件末尾追加新章节, CHK 编号从已有最大编号之后继续
     - 如果不存在: 创建新文件, CHK 编号从 CHK001 开始
   - 不同域名的检查清单始终是独立文件(如 `ux.md` 和 `security.md` 是两个文件)
...
**重要说明**: 每次 `/speckit.checklist` 命令调用都会基于域名生成检查清单. 同域名文件存在时追加新章节, 不同域名创建新文件. 这允许:
