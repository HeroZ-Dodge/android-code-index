...
按优先级依次尝试以下方式定位 feature 上下文:

**优先级 1: 从当前分支匹配**

1. 获取当前 Git 分支名称(`git rev-parse --abbrev-ref HEAD`)
2. 如果分支名匹配 `feature/{PM单号}_*` 格式:
   - 提取 PM 单号
   - 在 specs/ 下查找以该 PM 单号开头的目录(`specs/{PM单号}_*/`)
   - 检查该目录下是否存在 `feature.md`
   - 如果存在 → 读取 feature.md, 在 SP 点列表中匹配 $ARGUMENTS 对应的 SP 点, 进入"Feature 子目录模式"(步骤 1-NEW)

**优先级 2: 从 $ARGUMENTS 中的显式参数**

3. 如果 $ARGUMENTS 中包含 --pm-id 和 --spec-id 参数(用户直接指定):
   - 使用用户指定的参数
   - 在 specs/ 下查找以 --pm-id 开头的目录, 检查是否有 feature.md
   - 进入"Feature 子目录模式"(步骤 1-NEW)

**优先级 3: 搜索所有 feature.md**

4. 如果以上均未匹配, 搜索 specs/ 下的 feature.md(注: 此处不使用 `check-prerequisites.sh`, 因为 specify 需要查找 feature.md 而非 spec.md):
   ```bash
   find specs/ -name "feature.md" -maxdepth 2
   ```
   - 如果找到唯一一个 feature.md → 读取并匹配 SP 点, 进入"Feature 子目录模式"(步骤 1-NEW)
   - 如果找到多个 → 向用户展示列表并询问选择, 然后匹配 SP 点, 进入"Feature 子目录模式"(步骤 1-NEW)

**优先级 4: 无 Feature 上下文**

5. 如果未找到任何 feature.md 且无 PM 参数:
   - 进入"独立 Spec 模式"(步骤 1-STANDALONE)

---

### 步骤 1-NEW: Feature 子目录模式

1. 确认 PM 信息:
   - PM 父单号: 从 feature.md 元数据提取
   - PM 子单号: 从匹配的 SP 点提取
   - Spec 标题: 从 SP 点的名称提取
   - Feature 目录: feature.md 所在目录的绝对路径

2. **确定目录名和分支名**(优先使用文档中已有的信息):

   a. **优先读取 feature.md 中的已有定义**: 检查匹配的 SP 点中是否包含以下字段:
      - **Spec 目录**: 如 `270280_视频播放器相关达尔文实验代码下线/`
      - **Spec 分支**: 如 `feature/269382_270280_video-player-exp-cleanup`

   b. **如果 SP 点中已定义了 "Spec 目录" 和 "Spec 分支"**:
      - 从 "Spec 分支" 中提取英文短名(分支名最后一段, 即去掉 `feature/{PM父单号}_{PM子单号}_` 前缀后的部分)
      - 使用提取到的英文短名作为 `--short-name` 参数

   c. **如果 SP 点中未定义这些字段**(兜底生成):
      - 根据 SP 点的功能描述总结出一个 2-4 个词的英文短名(如 `player-upgrade`, `recommend-redesign`), 仅使用小写字母、数字和连字符

3. 调用脚本创建 spec 子目录和分支:
   ```bash
   .specify/scripts/bash/create-new-feature.sh --json \
     --mode spec \
     --pm-id "{PM父单号}" \
     --spec-id "{PM子单号}" \
     --spec-title "{清理后的Spec标题}" \
     --short-name "{英文短名(优先从文档提取, 否则自动生成)}" \
     --parent-dir "{feature目录绝对路径}" \
     "{功能描述}"
   ```

4. 解析 JSON 输出获取 BRANCH_NAME、SPEC_FILE、FEATURE_DIR

5. 继续到步骤 3(加载模板、填充 spec、验证质量)

---

### 步骤 1-STANDALONE: 独立 Spec 模式

没有找到 feature.md 上下文时, 询问用户是否有 PM 工单:

1. **询问用户**:

   使用 AskUserQuestion 工具询问:
   ```
   未检测到 feature 上下文(specs/ 下无 feature.md). 请选择创建方式:
   ```
   选项:
   - **有 PM 工单** — 提供 PM 工单链接或单号, 将使用 PM 单号命名目录和分支
   - **传统编号模式** — 使用自增编号(001, 002...)命名

2. **如果用户选择"有 PM 工单"**:

   a. 请用户提供 PM 工单链接或单号

   b. 使用 Skill 工具调用 `pm-netease:pm-skill` 解析 PM 工单, 提取:
      - PM 单号
      - PM 标题

   c. **重要: 独立模式下始终创建扁平目录**:
      - 无论用户提供的 PM 工单是否有父单关联, 都使用**该工单本身的单号和标题**作为目录名
      - **禁止**因为检测到父子单关系而使用 `--mode spec` 或 `--parent-dir` 创建嵌套目录
      - 独立模式的目标是在 `specs/` 下创建一个**顶层独立目录**, 如 `specs/{PM单号}_{标题}/`
      - 只有在"Feature 子目录模式"(步骤 1-NEW)中才允许嵌套目录结构

   d. **生成英文短名**: 根据功能描述总结出一个 2-4 个词的英文短名(如 `manga-upgrade`, `feed-redesign`), 仅使用小写字母、数字和连字符.

   e. 调用脚本创建 feature 目录和分支(**必须使用 `--mode feature`**):
      ```bash
      .specify/scripts/bash/create-new-feature.sh --json \
        --mode feature \
        --pm-id "{用户提供的PM单号}" \
   ...
   f. 解析 JSON 输出获取 BRANCH_NAME、FEATURE_DIR

   g. 在 FEATURE_DIR 下直接创建 spec.md(不创建子目录, 因为没有 feature.md 的 SP 拆分):
      - 复制 spec-template.md 到 `FEATURE_DIR/spec.md`
      - 继续到步骤 3(加载模板、填充 spec、验证质量)
...
2. **在创建新分支之前检查现有分支**:
   ...
   d. 使用计算出的编号和简短名称运行脚本 `.specify/scripts/bash/create-new-feature.sh --json "$ARGUMENTS"`:
      ...
      - 示例: `.specify/scripts/bash/create-new-feature.sh --json --number 5 --short-name "user-auth" "添加用户认证"`
...
以下步骤适用于所有模式(Feature 子目录模式、独立 Spec 模式的 PM 路径、传统编号模式). 此时 SPEC_FILE 和 FEATURE_DIR 已由前面的步骤确定.

**步骤 2.5: 归档本地需求文档**

如果用户在 $ARGUMENTS 中提供了**本地文件路径**(如 PRD 文档、截图、设计稿导出文件等), 将这些文件复制到需求目录的 `docs/` 子目录下:

1. 从 $ARGUMENTS 中识别所有本地文件路径(以 `/` 或 `~/` 开头, 或相对路径指向实际存在的文件)
2. 如果存在本地文件:
   ```bash
   mkdir -p "{FEATURE_DIR}/docs"
   cp "{本地文件路径1}" "{FEATURE_DIR}/docs/"
   cp "{本地文件路径2}" "{FEATURE_DIR}/docs/"
   ```
3. 同时使用 Read 工具读取这些文档内容, 作为生成 spec 的参考输入

**注意**: 仅复制用户提供的**本地文件路径**, 不下载在线链接的内容.
