...
1. 加载位于 `.specify/memory/constitution.md` 的现有章程文件.
   - 检查文件状态:
     - 如果文件包含 `[ALL_CAPS_IDENTIFIER]` 形式的占位符标记: 进入**模板填充模式**, 识别并替换所有占位符
     - 如果文件已包含具体内容(无占位符): 进入**更新模式**, 直接对现有内容进行增量修改
   **重要提示**: 用户可能需要比现有文件中使用的原则更少或更多. 根据用户输入相应地更新文档.
...
4. 一致性传播检查清单(将先前的检查清单转换为主动验证):
   ...
   - 读取 `.claude/commands/speckit.*.md` 中的每个命令文件(包括此文件)以验证在需要通用指导时没有过时的引用(如仅限 CLAUDE 的代理特定名称).
